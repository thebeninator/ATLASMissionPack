...
public class WingedReaper : CustomMission
{
        ...
        get
        {
            return new CustomMissionData()
            {
                ...
                DescriptionBluFor = String.Join("\n\n",
                    ...
                    "Enemy: Unknown quantities of AFVs and trucks",
                    "Friendly: 1x AC-130E",
    ...
    public override void OnMissionStartedLoading() {
        if (!M61SoundManager.sounds_created)
        {
            ...
            corSystem.createSound(Path.Combine(MelonEnvironment.ModsDirectory + "/ac130 assets", "m61burst.ogg"), MODE._3D_INVERSEROLLOFF | MODE.LOOP_NORMAL, out M61SoundManager.sound);
            M61SoundManager.sound.set3DMinMaxDistance(30f, 1300f);

            corSystem.createSound(Path.Combine(MelonEnvironment.ModsDirectory + "/ac130 assets", "m61closing.ogg"), MODE._3D_INVERSEROLLOFF, out M61SoundManager.sound_closing);
        ...
        AC130.Init();
        M61SoundManager.audio_manager = SettingsManager.Instance.GetComponent<AudioSettingsManager>();
    ...
    public override void OnMissionFinishedLoading()
    {
        ...
        Tools.SetStartingUnit(AC130.ac130.GetComponent<Vehicle>(), GHPC.Faction.Blue);

        Tools.CreatePlatoon("27th SOW, 16 SOS", AC130.ac130.GetComponent<Vehicle>());
